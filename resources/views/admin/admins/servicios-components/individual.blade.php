<div class="col-12 mt-4">
    ...
    <form method="POST" class="container" action="{{route('admin_servicios_individual_update', $data->slug)}}" enctype="multipart/form-data">
        @csrf
        <div class="d-flex justify-content-between mb-4">
            <h3>Servicios - {{$data->title}}</h3>
            <div>
                <button class="mdc-button mdc-button--unelevated filled-button--success mdc-ripple-upgraded" id="enviar_datos">Actualizar servicio</button>
                @if($data->status == 'public')
                    <a href="{{route('admin_servicios_reciclar_update', $data->id)}}" class="mdc-button mdc-button--unelevated filled-button--warning mdc-ripple-upgraded p-3" >Reciclar</a>
                @else
                    <a href="{{route('admin_servicios_publicar_update', $data->id)}}" class="mdc-button mdc-button--unelevated mdc-ripple-upgraded p-3" >Publicar</a>
                @endif
                <!-- Button trigger modal -->
                <a type="button" class="mdc-button mdc-button--unelevated filled-button--secondary mdc-ripple-upgraded p-3" data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$data->id}}">Eliminar</a>
                ...
                <div class="modal fade" id="staticBackdrop{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        ...
                        <div class="modal-footer">
                            ...
                            <a href="{{route('admin_servicios_eliminar', $data->id)}}" class="mdc-button mdc-button--unelevated filled-button--secondary mdc-ripple-upgraded p-3">
                                Aceptar y Eliminar
                            </a>
                        </div>
                        </div>
        ...
        @if(session('save'))
            ...
            <div class="alert alert-success  fade show d-flex justify-content-between" role="alert">
                <p class="pe-4 m-0">El registro se actualizo con exito</p>
            ...
            <div id="form" method="post" action="{{ route('about_admin_update') }}" enctype="multipart/form-data">
                ...
                <div class="mb-3 col-12">
                    ...
                    <select class="form-control" name="positicion">
                        <option value="0" @if(!$data->posicion) selected @endif>Insertar de primero (Por defecto)</option>
                        @for($i = 1; $i <= $count_servicios; $i++)
                            <option value="{{$i}}"  @if($data->posicion == $i) selected @endif>{{$i}}</option>
                        @endfor
                    </select>
                    @error('title')
                    <div id="validationServer03Feedback d-block" class="invalid-feedback">
